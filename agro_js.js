...
// JavaScript code to update the product price
//setInterval(() => {
// Fetch the latest price from the server-side script or API
//const latestPrice = getPriceFromServer();

// Update the HTML element with the latest price
//document.getElementById('product-price').textContent = latestPrice;
//}, 1000); // Update the price every 1 second
setInterval(() => {
    // Fetch the latest price from the third-party service
    fetch('https://api.third-party-service.com/product-price?key=YOUR_API_KEY')
